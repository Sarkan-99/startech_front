            ...
            <Card className="h-4/5 w-1/2 p-0 overflow-hidden">
                ...
                {/* Button */}
                    <div className={`relative w-full flex justify-end mt-4 `}>
                            <Link href={hello ? "/home" : "#"} passHref className={`${hello ? 'hover:cursor-pointer' : 'pointer-events-none'}`} >
                                <Button className={`mr-5 ${!hello ? 'disabled-link' : ''}`} label="J'accepte" severity="success" disabled={!hello} />
                            </Link>
                    </div>
                </div>
            </Card>
